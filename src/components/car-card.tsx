...
export function CarCard({ car }: CarCardProps) {
  ...
  return (
    <article className="card-surface overflow-hidden rounded-[24px]">
      <div className="relative aspect-[16/10] overflow-hidden">
        ...
        <div className="absolute left-5 top-5 rounded-full border border-white/14 bg-black/35 px-3 py-1 text-xs text-white/88 backdrop-blur">
          {car.heroTag}
        </div>
        <div className="absolute bottom-5 left-5 right-5 flex items-end justify-between gap-4">
          ...
          <div className="rounded-full bg-[var(--gold)] px-4 py-2 text-sm font-medium text-black">
            {formatCurrency(Number(car.price))}
          </div>
        </div>
      </div>

      <div className="grid gap-6 px-5 py-5 text-sm text-white/70 md:grid-cols-[1.2fr_1fr]">
